...
if [[ "${SKIPUPDATE,,}" != "true" ]]; then
    ...
    echo "STEAMAPPID=$STEAMAPPID \, STEAMBETAFLAG=$STEAMBETAFLAG"
    FEXInterpreter /home/steam/steamcmd/steamcmd.sh +force_install_dir "/config/gamefiles" +login anonymous +app_update "$STEAMAPPID" -beta "$STEAMBETAFLAG" validate +quit
...
FEXInterpreter /config/gamefiles/FactoryServer.sh -Port="$SERVERGAMEPORT" -ReliablePort="$SERVERMESSAGINGPORT" -ExternalReliablePort="$SERVERMESSAGINGPORT" "${ini_args[@]}" "$@" &
